...
y22 = [(i**(2/3))/2.7 for i in x22]
# y3 = [(i**0.418)/3 for i in x3]
y3 = [(i**(2/3))/3.1 for i in x3]
y4 = [(i)/4.2 for i in x4]
y5 = [(i**(.418))/2.8 for i in x5]

# ax.loglog([(end-i)/(.1*ti) for i in snaps], min_r, 'ko-')
ax.loglog([(end2-i)/(10*ti) for i in snaps2], min_r2, 'ko', markerfacecolor='none')
...
ax.plot(x22,y22, 'b--', linewidth=3)
ax.plot(x3,y3, 'b--', linewidth=3)
ax.plot(x4,y4, 'b--', linewidth=3)
ax.plot(x5,y5, 'b--', linewidth=3)


# ax.plot([10,100], [0.589]*2, 'k--')
ax.plot([0.01,10], [0.277]*2, 'g--')

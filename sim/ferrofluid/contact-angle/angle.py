...
# file = 'dump.angle1'
# file = 'dump.angle2'
file = '0.05/2/1.25/sim.lammpstrj'
...
result = np.zeros(16)
# angle = np.zeros((num_frames-begin_frame)//50) - 400
for frame in range(begin_frame, num_frames):
    ...
    for i in range(0, numz):
        ...
        radii[i] = radius.mean()
    x = np.linspace(-numz,numz,2*numz-1)*dz
    f = np.concatenate((-np.flip(radii)+2*radii[0], radii[1:]))
    cs = CubicSpline(x,f)
    idd = frame//50 -begin_frame//50
    # angle[idd] = (np.arctan2(1,cs(0,1))*180/np.pi)
    result[:numz] += radii
